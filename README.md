...
```
##web方式安装：
web方式安装配置文件 install.php 。这个安装程序具有以下特点：

主要功能
四步安装流程：

步骤1：环境检查（PHP版本、扩展、目录权限等）
步骤2：数据库配置（连接测试、数据库导入）
步骤3：系统配置（网站名称、URL、管理员邮箱）
步骤4：安装完成
环境检查功能：

PHP版本检查（要求7.4+）
必需扩展检查（PDO、PDO MySQL、JSON）
推荐扩展检查（GD）
目录权限检查（config/、uploads/、logs/）
数据库操作：

自动测试数据库连接
导入database.sql文件
自动生成database.php配置文件
安全特性：
...
默认管理员账户
用户名： admin
密码： password （在database.sql中设置的hash密码）
安装程序会自动处理所有必要的配置文件创建和数据库初始化，让用户能够快速部署系统。
## 配置说明
